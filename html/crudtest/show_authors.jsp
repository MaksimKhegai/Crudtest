			...
			<c:forEach var="author" begin="0" items="${authorsArray}" varStatus="status">
				<tr>
		            ...
		            <td>${author.birthDate}</td>
	 		            ...
	 		            <td>
	 		            	<c:if test='${permissionsObject.authorPermissions[status.index].update}'>
								<aui:form action="<%=editAuthorURL%>" name="<portlet:namespace/>fm">
									<aui:fieldset>
										<aui:input name="AuthorIndex" value="${status.index}" type="hidden">
										</aui:input>
									</aui:fieldset>
						...
						<td>
							<c:if test='${permissionsObject.authorPermissions[status.index].delete}'>
								<aui:form action="<%=removeAuthorURL%>" name="deleteForm${status.index}">
									<aui:fieldset>
										<aui:input name="AuthorIndex" value="${status.index}" type="hidden">
										</aui:input>
									</aui:fieldset>
...
<c:if test='${permissionsObject.globalPermissions.add}'>
	<aui:button-row cssClass="crudtest-buttons">
		<aui:button onClick="<%= addAuthorFormURL.toString() %>" value="Add an author"></aui:button>
	</aui:button-row>
</c:if>